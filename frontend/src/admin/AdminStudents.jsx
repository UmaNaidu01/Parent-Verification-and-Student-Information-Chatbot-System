...
const AdminStudents = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        await axios.put(`http://localhost:5000/api/v1/admin/students/update/${selectedStudent._id}`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
        await axios.post('http://localhost:5000/api/v1/admin/students/add', formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
      setShowModal(false);
  ...
  const sendAttendanceAlert = async (regNumber) => {
    ...
    const token = localStorage.getItem('adminToken');
    try {
      await axios.post('http://localhost:5000/api/v1/admin/send-attendance-alert', { regNumber }, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDelete = async (id) => {
    ...
    const token = localStorage.getItem('adminToken');
    try {
      await axios.delete(`http://localhost:5000/api/v1/admin/students/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchStudents();
    } catch (err) {
